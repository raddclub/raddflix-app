...
def setup_logging(name: str = "raddhub") -> logging.Logger:
    ...
    try:
        ...
        pass
    _LOGGING_READY = True
    _apply_polling_filter()
    return log

# ── Noisy-endpoint filter (suppresses background polling spam from werkzeug) ──
_SILENT_PATHS = {
    "/api/keepalive/status",
    "/upload/api/all-accounts",
    "/upload/api/jobs",
    "/upload/api/jd-stats",
    "/api/ping",
    "/api/tunnel/status",
    "/api/health/badges",
    "/settings/api/services/status",
    "/api/health",
}

class _PollingFilter(logging.Filter):
    """Drop werkzeug access-log lines for background-polling endpoints."""
...
def _apply_polling_filter() -> None:
    """Call once after setup_logging() to silence noisy background endpoints."""
    wz = logging.getLogger("werkzeug")
    # Only add if not already added
    if not any(isinstance(f, _PollingFilter) for f in wz.filters):
        wz.addFilter(_PollingFilter())
