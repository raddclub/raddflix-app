...
# ─────────────────────────────────────────────────────────────────────────────
# Doctor: JazzDrive session guardian
# ─────────────────────────────────────────────────────────────────────────────
_SG_ALERT_KEY = "session_guardian_last_alert"   # settings key — epoch of last WA alert

def _session_guardian() -> dict:
    """Lightweight session health check for the flix upload account.

    Does ONE read-only probe (get_storage_info) to verify the JSESSIONID is
    still alive.  Sends a WhatsApp alert:
      • immediately if the session probe fails (session dead)
      • once per day if token expires within 7 days

    Deliberately never uploads or writes to JazzDrive — zero suspension risk.
    """
    try:
        from . import db as _db, uploader as _up
        acct = _up.get_active_account()
        if not acct:
            return {"ok": False, "reason": "no flix account"}
        ...
        now      = time.time()
        exp_days = max(0, int((exp - now) / 86400)) if exp else 999

        # ── 1. Probe the session ──────────────────────────────────────────────
        session_alive = False
        if jsid and vk:
            try:
                # get_storage_info is a single GET — lightest possible probe
                info = _up.get_storage_info(vk, jsid, account_id=aid)
                session_alive = not info.get("error")
            except Exception:
                session_alive = False
        elif not jsid:
            # No session at all — that's a problem but not our job to alert every 45 min
            return {"ok": False, "reason": "no jsessionid"}

        # ── 2. Decide whether to send a WA alert ─────────────────────────────
        last_alert = float(_db.setting(_SG_ALERT_KEY) or "0")
        hours_since = (now - last_alert) / 3600

        msg = None

        if not session_alive:
            # Session dead — alert immediately (but no more than once per hour)
            if hours_since >= 1:
                msg = (
                    f"🚨 *RaddFlix — JazzDrive Session DEAD*\n"
                    f"Account: {msisdn}\n"
                    f"The session cookie has expired and uploads are now *paused*.\n"
                    f"Open the admin panel → Upload → paste new cookies from cloud.jazzdrive.com.pk"
                )
                log.warning("session_guardian: session dead for account %s — WA alert sent", aid)
        elif exp_days <= 7:
            # Expiring soon — alert once per day
            if hours_since >= 20:
                urgency = "🔴" if exp_days <= 2 else "🟡"
                msg = (
                    f"{urgency} *RaddFlix — JazzDrive Session Expiring*\n"
                    f"Account: {msisdn}\n"
                    f"Session expires in *{exp_days} day(s)*.\n"
                    f"Re-paste cookies soon: Admin panel → Upload → Paste cookies"
                )
                log.info("session_guardian: session expiring in %d days — WA alert sent", exp_days)

        if msg:
            _notify_admins(msg)
            _db.set_setting(_SG_ALERT_KEY, str(int(now)))
        ...
        log.debug("session_guardian error: %s", e)
        return {"ok": False, "error": str(e)}
...
_SCHEDULE = [
    ...
    ("session_guardian",_session_guardian, 2700),  # every 45 min
]
