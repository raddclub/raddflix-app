...
def _scan_worker(account_id: int, legacy_id: int) -> None:
    ...
    try:
        # ── STARTUP: prune old scan logs per retention setting ───────────────
        try:
            _retention_days = int(db.setting("log_retention_days") or "7")
            _pruned = db.cleanup_old_scan_logs(days=_retention_days)
            if _pruned:
                db.append_scan_log(account_id, "info",
                    f"[Housekeeping] Pruned {_pruned} log entries older than {_retention_days} day(s)")
        except Exception:
            pass
        ...
        if noise_count:
            cb({"type": "info", "message": f"Filtered out {noise_count} non-media file(s) (personal photos, docs, temp files)"})
        
        cb({"type": "scan_done", "message": f"JazzDrive scan complete — {len(filtered_files)} media file(s) discovered"})
        ...
        if skipped_count:
            cb({"type": "info",
                "message": f"Skipped {skipped_count} already-enriched file(s) — metadata is current, no re-lookup needed"})
        ...
        if needs_enrich:
            cb({"type": "enrich", "message": f"Metadata enrichment complete — {titles_done} new title(s) identified and saved"})
        ...
        cb({"type": "mirror", "message": f"Catalog synced — {mirrored} file(s) pushed to GitHub mirror"})
        ...
        try:
            ...
            cb({"type": "warn", "message": f"Scan stopped by user — {saved} file record(s) saved before stopping"})
        except Exception:
