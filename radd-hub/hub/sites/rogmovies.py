"""
RogMovies site plugin — v3.0 (Pure HTTP + Smart Ranking)

Architecture:
- Wraps hub.scrapers.rogmovies (Pure HTTP logic with v5.6 alignment)
...
from hub.scrapers import rogmovies as logic

log = logging.getLogger("hub.sites.rogmovies")


class RogMoviesPlugin(SitePlugin):
    name        = "RogMovies"
    description = "rogmovies.blog — Bollywood/Indian (Hindi/Dual Audio) via Pure HTTP (v3.0)"
    version     = "3.0"
    domain_keys = ["domain_rogmovies"]
    ...
    def search(self, page, movie_name: str, config: dict, check_control=None, log_fn=None) -> str:
        ...
        if log_fn:
            log_fn(f"rogmovies: Searching for '{movie_name}' year='{year_hint or 'any'}'...")
        ...
        if not results:
            raise RuntimeError(f"No results found on RogMovies for '{movie_name}'.")
        ...
        for candidate in results[:5]:
            ...
            if ok:
                if log_fn:
                    log_fn(f"rogmovies: Match found ({msg}) → '{candidate.get('title', '?')}'")
                return url
            else:
                if log_fn:
                    log_fn(f"rogmovies: Candidate rejected ({msg}) → {url[:40]}...")

        raise RuntimeError(f"No relevant results on RogMovies for '{movie_name}' after validation.")
    ...
    def get_download_link(self, page, movie_url: str, config: dict,
        ...
        if log_fn:
            log_fn(f"rogmovies: Analyzing post links: {movie_url}")
        ...
        if not links:
            raise RuntimeError(f"No download links found on RogMovies page: {movie_url}")
        ...
        if log_fn:
            log_fn(f"rogmovies: Selected: {best.get('label', 'Link')} → {target_url[:60]}...")
    ...
    def get_bridge_link(self, page, bridge_url: str, config: dict,
        ...
        if log_fn:
            log_fn("[RogMovies] Resolving bridge/cdn mirrors…")
        ...
        if "nexdrive." in bridge_url.lower():
            try:
                ...
                if log_fn: log_fn(f"rogmovies: nexdrive error: {e}")
        ...
        raise RuntimeError("RogMovies: Failed to resolve a stable download link.")
