...
## 2026-07-08 (later) — Implemented the mechanical fixes from the audit
- **Motion tokens aligned to spec** (`lib/design_system/motion/radd_motion.dart`): `RaddMotion.pulseDuration` now always returns 1800ms (was 800/1200ms split by tier — tier gating affects the scale component per Volume III, not loop duration); `RaddMotion.tune` now uses `Curves.easeOutBack` (spring-style overshoot) instead of `Curves.easeOutCubic`, matching "200ms spring, 8% overshoot." Chose to change the **code** to match the docs, not the reverse — the doc's Pulse/Tune values are tied to specific documented rules (reduced-motion disables `Tune` overshoot specifically, tier gating drops `Pulse` scale not duration) that the previous implementation didn't reflect, so the spec reads as the deliberate design intent.
- **Player HUD auto-hide**: `_scheduleHide()` changed from 4s to 3s per Volume X.
- **Player panel width**: `_openRightPanel`'s default `widthFactor` changed from 0.55 to 0.4 per the Volume X 40%-surface rule.
- **Correction to the earlier audit**: re-checked every `_player.pause()` call site in `player_screen.dart` while making these fixes. None of them fire when a settings/subtitle/audio panel opens — the 3 cited lines were app-backgrounded lifecycle, an explicit user tap-to-pause in immersive mode, and a sleep-timer expiry, all intentional. **The "opening a panel blocks playback" finding was a false positive; no fix was needed or made for that item.**
- **Not done in this pass — flagged as its own decision:** consolidating the Player's 12-14 persistent HUD controls down to "5 + one More sheet" is a full UI restructuring (moving most controls into a sheet, redesigning the top/bottom bars), functionally the same scope as the already-tracked `PLAYER-CONSOLIDATION` task. Doing that without a Flutter toolchain to compile/widget-test in this environment carries meaningfully higher risk than the mechanical fixes above, so it was intentionally left for a dedicated pass rather than bundled in here.
