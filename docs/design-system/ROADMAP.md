...
- [x] **Phase 3 — Shared Components** — `RaddButton`, `RaddCard`, `RaddSheet`, `RaddBanner`, `RaddTextField`, `RaddChip`, `RaddLockPad`, `SettingsRow` built in `lib/design_system/components/` (Volumes IV, VIII). All 🚧 In Progress — not yet used by any screen, no widget tests written yet, no Flutter toolchain available in this environment to compile-check or run tests. Verify build + write tests before flipping any to ✅ Production.
