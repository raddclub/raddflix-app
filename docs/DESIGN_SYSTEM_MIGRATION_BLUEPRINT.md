...
| Local Media / Local Folder | `local_media_screen.dart`, `local_folder_screen.dart` | 1,102 / 1,250 | 33 / 51 | 6 / 0 | 55 / 71 | 0 / 0 | ~25% | `local_folder_screen.dart` has the highest `AppColors.*` count of any screen (51). |
| Vault / Vault Settings | `vault_screen.dart`, `vault_settings_screen.dart` | 671 / 459 | 17 / 15 | 2 / 0 | 25 / 17 | 0 / 0 | ~25% | No wireframe; general token rules only. |
...
| `RaddTextField` | Any remaining raw `TextField`/`TextFormField` outside Login/Register (e.g. Search bar, Add/Edit Profile name field) | 3 (Login, Register) | Phase 3/5 |
