  ...
  List<String> _extractGenres(List<CatalogItem> all) {
    ...
    for (final item in all) {
      ...
      if (raw.trimLeft().startsWith('[')) {
        // Fix #10: compile once per call, not once per item (the for-loop is inside this call)
        const genreCleanPattern = r'^\[|\]
...
class _SnippetText extends StatelessWidget {
  ...
  // Fix #9: static — avoids re-compiling the regex on every build() call
  // (this widget is instantiated once per search result row).
  ...
  Widget build(BuildContext context) {
    ...
    int cursor = 0;
    for (final m in _re.allMatches(raw)) {
      if (m.start > cursor) {
...
;
        final inner = raw.trim().replaceAll(RegExp(genreCleanPattern), '');
        parts = inner.split(',').map((s) => s.trim().replaceAll('"', '').replaceAll("'", '')).toList();
      } else {
        parts = raw.split(',');
      }
