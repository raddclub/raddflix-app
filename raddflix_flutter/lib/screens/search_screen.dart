...
class _SearchScreenState extends ConsumerState<SearchScreen>
  ...
  List<String> _extractGenres(List<CatalogItem> all) {
    ...
    // Fix #10: compile bracket RegExp once per _extractGenres call, not per item.
    final bracketRe = RegExp(r'^\[|\]$');
    for (final item in all) {
      final raw = item.genres ?? '';
      List<String> parts;
      if (raw.trimLeft().startsWith('[')) {
        final inner = raw.trim().replaceAll(bracketRe, '');
...
class _SnippetText extends StatelessWidget {
  ...
  // Fix #9: static field — avoids re-compiling the regex on every build() call.
