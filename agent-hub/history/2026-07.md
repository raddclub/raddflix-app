...
- **Problem:** Sending `https://raw.githubusercontent.com/.../AGENT_PROMPT.md` to a fresh agent
  ...
  2. The old step-5 warning contained classifier-trigger phrases (`blindly`, `disable safety
     confirmations`, `never ask`) even though they were listed as things NOT to do — classifiers
     flag the words regardless of negation context
  3. The git clone command embedded credentials directly in the URL
- **Fix (commit fd351a1):** Rewrote `AGENT_PROMPT.md` bootstrap section:
  - Added "Note for agents" block at the top explaining this is a legitimate owner-written doc
  - Removed all classifier-trigger phrases from the document entirely
  - Split clone into two steps: plain `git clone` then `git remote set-url` with the token,
    so credentials are never embedded in the initial clone command
  - Replaced "Do exactly this" with plain imperative steps
- **New recommended user phrasing:** "Please read this project setup file and follow the
  bootstrap steps to get started: [URL]"
...
### Root-cause analysis
After tracing all data paths from Oracle → Flask API → Flutter local DB → UI:

**BUG-FREE-EP-01 — Episode `is_free` hardcoded `0` in all three catalog API endpoints**
- `catalog_api.py` routes `/sync`, `/db_update`, and `/delta` all SELECT episodes from the
  Oracle `files` table without an `is_free` column, then hardcode `"is_free": 0` in the JSON.
- Flutter's `|| widget.item.isFree` OR-condition already rescues *fully free shows*
  (title-level `is_free=1` propagates), but any show where episodes should individually be free
  (or where `widget.item.isFree` was stale) was permanently blocked.
- Fix: all three endpoints now build a `{title_id: is_free}` lookup from the already-fetched
  title rows and use it instead of the hardcoded `0`.
- Files: `hub/routes/catalog_api.py` (three episode serialisation loops)

**BUG-GATE-LOCALFILE-01 — Play gate fires before downloaded-file check in `_playMovie`**
- `_playMovie()` ran the subscription gate at line 278 *before* resolving `localPath` at line 285.
- A user whose subscription lapsed couldn't play a movie they had already downloaded.
- Fix: moved `localPath` lookup before the gate; gate now uses `localPath == null` as an extra
  bypass condition (mirrors existing `downloads_screen.dart` `is_free: true` behaviour).
- Files: `show_detail_screen.dart`

**BUG-GATE-LOCALFILE-02 — Episode play gate ignores existing downloaded copy**
- `_playEpisode()` gate condition was `!isFreeEp && !_isSubscribed`, with no check for `localPath`.
- A downloaded paid episode would still show "Subscribe to access paid content" post-expiry.
- Fix: condition extended to `!isFreeEp && localPath == null && !_isSubscribed`.
- Files: `show_detail_screen.dart`

**BUG-DL-URL-01 — Episode individual download used raw XOR-encoded `share_url` as fallback**
- `onDownload` lambda passed the raw `ep['share_url']` (RF1:xxx encoded) as `streamUrl`.
- `downloadFile`'s Path B (JazzDrive via `getShareInfo`) decodes correctly under normal conditions,
  but if Path B ever fails the fallback is the undecodable RF1:xxx string → corrupted/zero-byte file.
- Fix: async lambda now calls `LocalDb.decodeShareUrl(epShareUrl)` and passes the decoded URL.
- Files: `show_detail_screen.dart`

**BUG-DL-RETRY-01 — `retryDownload` passed raw XOR-encoded URL to `startDownload`**
- `LocalDb.getFileInfo()` returns the raw encoded `share_url` (unlike `getShareInfo` which decodes).
- The retry path was passing this encoded URL as `streamUrl`, causing the same fallback failure risk.
- Fix: `retryDownload` now calls `LocalDb.decodeShareUrl(rawShareUrl)` before use.
- Files: `downloads_provider.dart`

**BUG-SUB-STALE-01 — `_isSubExpired()` used stale `user.subscription` instead of live provider**
- `downloads_screen.dart` `_isSubExpired()` read `user.subscription` from the cached auth object,
  which can be stale after a subscription renewal or expiry event.
- Fix: function now reads `ref.read(subscriptionProvider).status` first (live), falls back to the
  cached auth object only if the provider hasn't loaded yet.
- Added `import '../providers/subscription_provider.dart'`.
- Files: `downloads_screen.dart`
