...
| ID | Changed | Summary |
|----|---------|---------|
| ✅ TASK-UI-12 | `actor_service.dart`, `cast_rail.dart`, `actor_screen.dart`, `watchlist_screen.dart`, `search_screen.dart`, `bottom_nav.dart`, `downloads_screen.dart` | **12 UI/UX improvements.** (1) Cast Rail: TVMaze API for TV shows (2-4 → up to 20 cast with character names + photos), cards 74→84px, photo 64→72px, char label 9→11px, shimmer 5→10 circles, See All bottom sheet for >10 members, Wikipedia bio on actor screen. (2) Actor screen: collapsing header, bio section, filmography grid. (3) Watchlist: Sort bottom sheet — Date Added / A–Z / Rating / Type. (4) Search empty state: illustrated icon container, clear-search / clear-filters CTAs, helpful copy. (5) Bottom nav: wider pill (44→52px) on active, glow shadow, animated dot indicator, AnimatedDefaultTextStyle label. (6) Downloads empty state: animated pulsing icon, feature pills (offline/resume), "Browse Content" gradient CTA. (Already confirmed implemented: Continue Watching row, progress bars, hero dots, watched checkmarks, rating badge, profile avatar.) |
