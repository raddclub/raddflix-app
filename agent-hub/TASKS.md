...
| TASK-021 | ✅ DONE | 2026-06-07 | Re-scan missing metadata: Flask endpoint POST /api/admin/rescan-metadata + admin panel button. Finds files with title_id=NULL and re-runs IMDb-first enrich chain. |
| TASK-022 | ✅ DONE | 2026-06-07 | Player Pass 1 critical bugs: BUG-01 (volume boost), BUG-02 (audio session leak), BUG-03 (double retry race), BUG-04 (cast stale URL), BUG-05 (sleep volume), BUG-06 (track memory), BUG-07 (bg play flag), BUG-14 (resume position), LAYOUT-01 (SkipBtn Positioned), LAYOUT-02 (More Panel overflow), UX-01 (Night Mode wiring) |
