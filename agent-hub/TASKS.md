...
| PLAYER-CONSOLIDATION (RaddSheet) | Merge the Player's **7 real inline panel classes** in `player_screen.dart` (`_SubtitlePanel`, `_AudioTrackPanel`, `_VideoZoomPanel`, `_AudioEffectPanel`, `_QuickShortcutsPanel`, `_SettingsPanel`, `_SidebarCustomizerPanel`) into one `RaddSheet` with 3 tabs (Playback / Audio & Video / Extras). Deliberately last in the whole design-system rollout — `player_screen.dart` is 9,372 lines, the single highest blast-radius file in the codebase. Depends on: token layer + `RaddSheet` primitive existing first. The 50 dead files in `widgets/player/` remain on disk (deletion declined by user) — exclude them from the working-file inventory when planning this merge; do not confuse them with the 7 real inline panels. | `docs/design-system/IMPLEMENTATION_PLAN.md` sections 2 (RaddSheet row) and 3 (Player row) | ⏳ IN PROGRESS — Steps 1–3 done (commits d9da81f, dae6c3c, 663f475): (1) fixed `radd_sheet.dart` missing `radd_type` import, `_VideoZoomPanel` removed + `_openZoomPanel` → `RaddSheet.show`; (2) `_AudioTrackPanel.build()` header stripped, `_openAudioPanel` → `RaddSheet.show`; (3) `_QuickShortcutsPanel.build()` header stripped, `_openMoreMenu` → `RaddSheet.show`. **Remaining 4 panels need bounded-height handling and should be tackled as a single pass when building the final 3-tab unified player sheet:** `_AudioEffectPanel` (EQ vertical sliders need fixed height), `_SettingsPanel` (4-tab ListView each), `_SubtitlePanel` (6-tab, 500+ lines state), `_SidebarCustomizerPanel` (ReorderableListView needs bounded height). Next action: build the `_openPlayerSheet()` function that calls `RaddSheet.show(style: tabbed, tabs: [Playback/Audio & Video/Extras])` and routes each panel's header-stripped content into the correct tab. |
