...
| L5 — ZOOMMODE-SAVEGAP | `_openZoomPanel` callback: `setState(() => _zoomMode = mode)` but `_savePrefs()` was never called — zoom mode preference was lost on every restart. Added `_savePrefs()` call. | pending | ✅ DONE |
